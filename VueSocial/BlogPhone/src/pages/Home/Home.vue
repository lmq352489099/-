<template>
  <div id="msite">
    ...
    <div class="pyq-lists">
      ...
      <scroll
        @change="showupload=true"
        @refresh="getPyqLists(1)"
        :stop="55"
        :threshold="70"
        ref="scroll"
        class="scroll-wrap pyq-card"
        :data="PyqList"
      >
        <ul>
          <!-- :key="Math.random()" -->
          <li class="pyq" :key="index" v-for="(list,index) in PyqList">
            <div
              class="del"
              @click="del(list._id)"
              v-if="list.writer.username === userInfo.username"
            >&times;</div>
            ...
            <div class="content">
              <div
                class="username"
                @click="goToPindex(list.writer.username,list.writer._id)"
              >{{list.writer.username}}</div>
              <div class="pyq-content">{{list.content}}</div>
              <div style="width: 100%" v-if="list.pimg.length === 1" class="one-box">
                <div
                  class="one"
                  @click="showimg(list.pimg,0)"
              ...
              <div v-if="list.pimg.length > 1" class="pyq-img">
                <div
                  ref="items"
                  @click="showimg(list.pimg,index)"
                  :class="list.pimg.length === 4?'four':'nine'"
                  v-for="(item,index) in list.pimg"
                  :key="index"
              ...
              <div class="footer">
                ...
                <div @click="comment(list._id,list.writer.username)" class="comments">评论</div>
              </div>
              <div v-if="list.comments.length" class="comment-lists">
                <div class="comment">
                  <div
                    @click="commentTo(list._id,comment.from,list.writer.username)"
...
export default {
  computed: {
    ...mapState(["userInfo", "cunread", "isResize"])
  },
  mounted() {
    this.$nextTick(() => {
      this.initImg();
