        ...
        <ul>
          <li>
            <div class="news-box" @click="enterComList">
              ...
              <div class="content">评论
                <span v-if="cunread" class="cnews">{{cunread}}</span>
              </div>
            </div>
          </li>
                ...
                <div class="content">
                  <div class="username">
                    ...
                    <span
                      class="time"
                      v-if="item.addTime"
                    >{{item.addTime|chatTime}}</span>
                  </div>
                  <span class="unread" v-if="item.unread">{{item.unread}}</span>
                  <div class="tnew">{{item.content}}</div>
                </div>
              </div>
            </li>
          </template>
...
export default {
  computed: {
    ...mapState(["userInfo", "cmessage", "cunread", "chatList"])
  },
  activated() {
    this.$nextTick(() => {
  ...
  methods: {
    ...mapMutations(["clear_cunread", "set_chatList", "clear_chatUnread"]),
    enterComList() {
      let username = this.userInfo.username;
      this.axios
        .get("/comment/clearcomunread", {
          params: {
            username: username
          }
        })
